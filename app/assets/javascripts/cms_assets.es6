$(() => {
  ...
  $('#CMS_references_link').click(e => {
    ...
    $.ajax({
      ...
      $.ajax({
        ...
        $('#CMS_index_content').append(getReferenceLinkGrid(reference_links, reference_link_categories, response.users, type))
  ...
  function getReferenceLinkGrid(reference_links, reference_link_categories, users, type){
    return `<div>
              <h4 id='cms_reference_${type}_filter_header'><u>Find attached reference ${type} by category:</u></h4>
            </div>
            <div id='cms_reference_${type}s_filter_menu' class="button-group filter-button-group col-md-11">
            ...
            ${ _.map(reference_links, reference_link => {
                return `<div id="${reference_link.id}" class="col-md-12 reference_${type}_item ${_.isUndefined(reference_link_categories[reference_link.id]) ? 'Unassigned' : _.map(reference_link_categories[reference_link.id], link => { return link.category }).join(" ") }">
                    <div class='col-md-4'>
